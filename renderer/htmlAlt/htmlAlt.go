...
func (c *Config) SetOption(name renderer.OptionName, value interface{}) {
	...
	case optAltHtml:
		c.AltHtml = value.(bool)
	case optDocTitle:
		c.DocTitle = value.(string)
	}
}
...
// AltHtml is an option name used in WithAltHtml.
const optAltHtml renderer.OptionName = "AltHtml"

type withAltHtml struct {
}

func (o *withAltHtml) SetConfig(c *renderer.Config) {
	c.Options[optAltHtml] = true
}

func (o *withAltHtml) SetHTMLOption(c *Config) {
	c.AltHtml = true
}

// WithXHTML is a functional option indicates that nodes should be rendered in
// xhtml instead of HTML5.
func WithAltHtml() interface {
	Option
	renderer.Option
} {
	return &withAltHtml{}
...
// Title is an option name used in WithTitle.
const optDocTitle renderer.OptionName = "DocTitle"

type withDocTitle struct {
	value string
}

func (o *withDocTitle) SetConfig(c *renderer.Config) {
	c.Options[optDocTitle] = o.value
}

func (o *withDocTitle) SetHTMLOption(c *Config) {
	c.DocTitle = o.value
}

// WithDocTitle is a functional option indicates that nodes should be rendered
// with a div class  equals doctitle.
func WithDocTitle() interface {
	Option
	renderer.Option
} {
	return &withDocTitle{}
...
// method Renderer that initializes a map of menthods, each one rendering a particular kind of node (type of node)
...
// method to start rendering a document. possible future options could be dimension of a document page

	// nothing to do
	// replaced with <div class="mdfile">
	if entering {
		_, _ = w.WriteString("<div ")
// need to find file name
//rrr

		titleStr := r.Config.DocTitle
fmt.Printf("*** renderDoc *** title: %s\n", titleStr)

		if len(titleStr) > 0 {
			_, _ = w.WriteString("class=\"")
			_, _ = w.WriteString(titleStr)
			_, _ = w.WriteString("\"")
		}
		_, _ = w.WriteString(">\n")
...
// method to render a heading
	...
	if entering {
		...
		// no stated attribute, so create default attribute
			titleStr := r.Config.DocTitle
//fmt.Printf("*** renderDoc *** title: %s\n", titleStr)
...
func (r *Renderer) renderList(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		...
		if n.Attributes() != nil {
			...
			titleStr := r.Config.DocTitle
			if len(titleStr) > 0 {
				fmt.Fprintf(w, " class=\"%s_%s_NL%d\"", titleStr, tag, r.nest)
//				_, _ = w.WriteString("\"")
			}
		}
		_, _ = w.WriteString(">\n")
...
//			_, _ = w.WriteString("<li>")
			...
			if len(titleStr) > 0 {
				tag := "ul"
				if r.ordList {tag = "ol"}
				fmt.Fprintf(w, " class=\"%s_li%s_NL%d\"", titleStr, tag, r.nest)
			}
		}
		_ = w.WriteByte('>')
		fc := n.FirstChild()
...
func (r *Renderer) renderParagraph(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	if entering {
		...
		if n.Attributes() != nil {
			...
			titleStr := r.Config.DocTitle
			if len(titleStr) > 0 {
				fmt.Fprintf(w, " class=\"%s_p\"", titleStr)
			}
		}
		_ = w.WriteByte('>')
	} else {
...
func (r *Renderer) renderThematicBreak(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if n.Attributes() != nil {
		...
		titleStr := r.Config.DocTitle
		if len(titleStr) > 0 {
			fmt.Fprintf(w, " class=\"%s_hr\"", titleStr)
		}
	}
	if r.XHTML {
		_, _ = w.WriteString(" />\n")
...
func (r *Renderer) renderCodeSpan(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	if entering {
		...
		if n.Attributes() != nil {
			...
			titleStr := r.Config.DocTitle
			if len(titleStr) > 0 {
				fmt.Fprintf(w, " class=\"%s_code\"", titleStr)
			}
		}
		_ = w.WriteByte('>')
...
func (r *Renderer) renderEmphasis(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		if !r.AltHtml {
			...
			titleStr := r.Config.DocTitle
			if len(titleStr) > 0 {
				fmt.Fprintf(w, " class=\"%s_sp_%s\"", titleStr, tag)
			}
			_ = w.WriteByte('>')
		}
	} else {
